...
// Building the Trainer to xml block
  ...
  def statusToXml(status: StatusEffectStrategyContext): Elem = {
    status.strategy match {
      case normal: NormalState       => normalStateToXml(normal)
      case burned: BurnedState       => burnedStateToXml(burned)
      case poisoned: PoisonedState   => poisonedStateToXml(poisoned)
      case sleep: SleepState         => sleepStateToXml(sleep)
      case paralyzed: ParalyzedState => paralyzedStateToXml(paralyzed)
      case _                         => <status stateType="UnknownState"/>
    }
  }

  def normalStateToXml(status: NormalState): Elem = {
    <status stateType="NormalState">
    <name>{status.statusName}</name>
    <duration>{status.duration}</duration>
  </status>
  }

  def burnedStateToXml(status: BurnedState): Elem = {
    <status stateType="BurnedState">
    <name>{status.statusName}</name>
    <duration>{status.duration}</duration>
    <intensity>{status.intensity}</intensity>
  </status>
  }

  def poisonedStateToXml(status: PoisonedState): Elem = {
    <status stateType="PoisonedState">
    <name>{status.statusName}</name>
    <duration>{status.duration}</duration>
    <intensity>{status.intensity}</intensity>
  </status>
  ...
  def sleepStateToXml(status: SleepState): Elem = {
    <status stateType="SleepState">
    <name>{status.statusName}</name>
    <duration>{status.duration}</duration>
  </status>
  }

  def paralyzedStateToXml(status: ParalyzedState): Elem = {
    <status stateType="ParalyzedState">
    <name>{status.statusName}</name>
    <duration>{status.duration}</duration>
  </status>
  }
  ...
  def pokemonToXml(pk: Pokemon): Elem = {
    <pokemon>
        ...
        <status>{statusToXml(pk.status)}</status>
  </pokemon>
  }
...
///_____________________LOAD____________________
  ...
  def xmlToStatus(node: scala.xml.Node): StatusEffectStrategyContext = {
    val statusType = (node \\ "@stateType").text
    println(s"Extracted type attribute value: $statusType") // Print the extracted value for debugging
    val strategy = statusType match {
      case "NormalState"    => xmlToNormalState(node)
      case "BurnedState"    => xmlToBurnedState(node)
      case "PoisonedState"  => xmlToPoisonedState(node)
      case "SleepState"     => xmlToSleepState(node)
      case "ParalyzedState" => xmlToParalyzedState(node)
      case _                => new NormalState() // Fallback to normal state for unknown types
    }
    StatusEffectStrategyContext(strategy)
  }

  def xmlToNormalState(node: scala.xml.Node): NormalState = {
    NormalState()
  }

  def xmlToBurnedState(node: scala.xml.Node): BurnedState = {
    val duration = (node \\ "duration").text.toInt
    val intensity = (node \\ "intensity").text.toInt
    BurnedState(duration, intensity)
  }

  def xmlToPoisonedState(node: scala.xml.Node): PoisonedState = {
    val duration = (node \\ "duration").text.toInt
    val intensity = (node \\ "intensity").text.toInt
    PoisonedState(duration, intensity)
  }

  def xmlToSleepState(node: scala.xml.Node): SleepState = {
    val duration = (node \\ "duration").text.toInt
    SleepState(duration)
  }

  def xmlToParalyzedState(node: scala.xml.Node): ParalyzedState = {
    val duration = (node \\ "duration").text.toInt
    ParalyzedState(duration)
  }
  ...
  object GameStateLoader {
    ...
    def gameStateFromXml(xml: Node): GameState = {
      println(xml)
      val stype = (xml \\ "@type").text.trim // Extract type attribute and store it in a variable
      println(s"Extracted type attribute value: $stype") // Print the extracted value for debugging
      (xml \\ "@type").text match {
